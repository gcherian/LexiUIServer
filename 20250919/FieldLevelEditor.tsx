...
export default function FieldLevelEditor() {
  ...
  /* -------- Row click: local fuzzy → FAST server match → HEAVY merge -------- */
  function onRowClick(r: FieldRow) {
    ...
    // FAST only (fuzzy + tfidf)
    (async () => {
      try {
        if (!docId) return;
        const fast: MatchResp = await matchField(docId, r.key, r.value, { fast_only: true, max_window: 12 });
        const toRect = (m: any): EditRect | null =>
          !m ? null : ({ page: m.page, x0: m.rect.x0, y0: m.rect.y0, x1: m.rect.x1, y1: m.rect.y1 });
        const fastOverlays = [
          fast?.methods?.fuzzy ? { label: "fuzzy", color: COLORS.fuzzy, rect: toRect(fast.methods.fuzzy) } : null,
          fast?.methods?.tfidf ? { label: "tfidf", color: COLORS.tfidf, rect: toRect(fast.methods.tfidf) } : null,
        ].filter(Boolean) as {label:string;color:string;rect:EditRect|null}[];

        // If we don't have a visible page yet, jump to the first fast rect we have
        if (!rect) {
          const first = fastOverlays.find(o => o.rect);
    ...
    // HEAVY (minilm + distilbert) in background; merge results when ready
    matchField(docId!, r.key, r.value, { fast_only: false, max_window: 12 })
      .then((all) => {
        const toRect = (m: any): EditRect | null =>
          !m ? null : ({ page: m.page, x0: m.rect.x0, y0: m.rect.y0, x1: m.rect.x1, y1: m.rect.y1 });
        const heavyOverlays = [
          all?.methods?.minilm ? { label: "minilm", color: COLORS.minilm, rect: toRect(all.methods.minilm) } : null,
          all?.methods?.distilbert ? { label: "distilbert", color: COLORS.distilbert, rect: toRect(all.methods.distilbert) } : null,
        ].filter(Boolean) as {label:string;color:string;rect:EditRect|null}[];

        if (heavyOverlays.length) {
          setOverlays((prev) => {
            const by = new Map(prev.map(o => [o.label, o]));
            for (const o of heavyOverlays) by.set(o.label, o);
            return Array.from(by.values());
          });
        }
      })
      .catch((e) => {
        console.warn("HEAVY matchField failed", e);
      });
  }
