...
export default function FieldLevelEditor() {
  ...
  // NEW: model overlay rects
  const [overlays, setOverlays] = useState<Array<{page:number;x0:number;y0:number;x1:number;y1:number;color:string;label:string}>>([]);
  ...
  // row click → fetch boxes from /lasso/locate
  async function onRowClick(r: FieldRow) {
    ...
    try {
      ...
      for (const method of ["autolocate","tfidf","minilm","distilbert","layoutlmv3"] as const) {
        const h = (hits as any)[method];
        if (h && h.page != null && h.rect) {
          ov.push({ page: h.page, ...h.rect, color: COLORS[method], label: method });
        }
      }
      setOverlays(ov);

      // focus the dominant page (first overlay or 1)
      const pg = ov.length ? ov[0].page : 1;
      setPage(pg);

      // keep the editable rect as the autolocate (if present) else tfidf else null
      const prio = ["autolocate", "tfidf", "minilm", "distilbert", "layoutlmv3"] as const;
      let best: any = null;
      for (const m of prio) {
        if ((hits as any)[m]) { best = (hits as any)[m]; break; }
      }
      if (best) {
        setRect({ page: best.page, x0: best.rect.x0, y0: best.rect.y0, x1: best.rect.x1, y1: best.rect.y1 });
      } else {
        setRect(null);
      }
    } catch (e) {
      // Even if heavy models break server-side, autolocate/tfidf still return.
      console.warn("locateAll failed", e);
      setOverlays([]);
    }
  }
  ...
  async function onRectCommitted(rr: EditRect) {
    ...
    try {
      if (docId) {
        ...
        // Update KV list
        setRows(prev => prev.map(row => row.key===focusedKey ? { ...row, value: text, rects: [{ page:rr.page, x0:rr.x0, y0:rr.y0, x1:rr.x1, y1:rr.y1 }] } : row));
      }
    } catch {}
  }
  ...
  return (
    <div className="workbench">
      <div className="wb-toolbar" style={{ gap: 8 }}>
        <label className="btn">
          <input type="file" accept="application/pdf" onChange={onUploadPdf} style={{ display:"none" }} /> PDF
        </label>
        <label className="btn">
          <input type="file" accept="application/json" onChange={onUploadEcm} style={{ display:"none" }} /> ECM JSON
        </label>
        <input className="input" placeholder="...or paste /data/{doc_id}/original.pdf" value={docUrl} onChange={(e)=>setDocUrl(e.target.value||"")}
